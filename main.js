...
class FlappyBirdGame {
  ...
  _OnCreate(scene) {
    ...
    this._keys = {
      up: this._scene.input.keyboard.addKey(
  ...
  _OnUpdate(scene) {
    ...
    const timeElapsedInS = Math.min(
      (currentFrame - this._previousFrame) / 1000.0,
      1.0 / 30.0
    );
  ...
  _CheckGameOver() {
    const results = this._birds.map((b) =>
      this._IsBirdOutOfBounds(b)
    );
  ...
  _UpdatePipes(timeElapsed) {
    ...
    if (this._pipes[0].X + this._pipes[0].Width <= 0) {
      ...
      p.Reset(
        this._pipes[this._pipes.length - 1].X + _PIPE_SPACING_X
      );
      this._pipes.push(p);
    }
  }
